-- Preserve client-side encrypted journal data in full backups.

-- Preserve system timeline history in backup restore and prevent trigger duplication.
...
as $$
  select case
    ...
    then jsonb_build_object(
      ...
      'system_events',(select count(*) from public.system_events where user_id=p_user_id),
...
declare
  ...
  v_tag_links jsonb;
  v_connections jsonb;
  v_system_events jsonb;
  v_journal_vault jsonb;
...
begin
  ...
  v_system_events := case when jsonb_typeof(v_data->'system_events')='array' then v_data->'system_events' else '[]'::jsonb end;
  v_journal_vault := case when jsonb_typeof(v_data->'journal_vault')='object' then v_data->'journal_vault' else null end;
  v_journal_entries := case when jsonb_typeof(v_data->'journal_entries')='array' then v_data->'journal_entries' else '[]'::jsonb end;
  ...
  if jsonb_array_length(v_members) > 10000
     ...
     or jsonb_array_length(v_connections) > 10000
     or jsonb_array_length(v_system_events) > 1000000
     or jsonb_array_length(v_journal_entries) > 100000 then
  ...
  perform pg_catalog.set_config('nihility.suppress_timeline','on',true);
  ...
  delete from public.system_events where user_id = p_user_id;
  delete from public.journal_vaults where user_id = p_user_id;
  ...
  if v_journal_vault is not null then
    insert into public.journal_vaults(
      ...
      coalesce(v_journal_vault->>'cipher_suite','AES-256-GCM'),
      coalesce(v_journal_vault->>'kdf_name','PBKDF2-HMAC-SHA-256'),
      (v_journal_vault->>'kdf_iterations')::integer,
      v_journal_vault->>'kdf_salt',
      v_journal_vault->>'wrap_iv',
      v_journal_vault->>'wrapped_key',
      coalesce(v_journal_vault->>'recovery_kdf_name','HKDF-SHA-256'),
    ...
    select
      ...
      coalesce(nullif(item->>'created_at','')::timestamptz,pg_catalog.now()),
      coalesce(nullif(item->>'updated_at','')::timestamptz,pg_catalog.now())
    from jsonb_array_elements(v_journal_entries) item;
  elsif jsonb_array_length(v_journal_entries) > 0 then
    raise exception 'Journal entries require journal vault metadata';
  end if;
  ...
  return jsonb_build_object(
    ...
    'system_events', jsonb_array_length(v_system_events),
