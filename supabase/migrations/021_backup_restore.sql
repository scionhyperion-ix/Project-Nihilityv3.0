-- Atomic, owner-only restore primitives for portable Nihility backups.
-- The browser never calls these directly. They are service-role-only helpers
-- used by the authenticated nihility-secure Edge Function.
...
revoke all on function public.nihility_backup_counts(uuid)
  from public, anon, authenticated;
grant execute on function public.nihility_backup_counts(uuid)
  to service_role;
