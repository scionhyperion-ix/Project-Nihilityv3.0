...
(()=>{
  ...
  function installTimelineRoute(){
    if(!document.querySelector('#timelineRoute')){
      ...
      route.innerHTML='<article class="panel section-panel system-timeline-panel"><div class="panel-heading split-heading"><div><p class="eyebrow">System history</p><h3>Timeline</h3></div><span id="timelineResultCount" class="soft-pill">0 events</span></div><p class="muted timeline-intro">A chronological view of fronting, member lifecycle, groups, relationships, imports, syncs, and restores. Private notes, mood, context, credentials, and security logs are not copied into timeline events.</p><div class="timeline-toolbar"><label class="search-field"><span>⌕</span><input id="timelineSearch" type="search" placeholder="Search timeline" autocomplete="off"></label><label>Category<select id="timelineCategoryFilter"><option value="all">All activity</option><option value="fronting">Fronting</option><option value="members">Members</option><option value="groups">Groups</option><option value="relationships">Relationships</option><option value="integrations">Integrations</option><option value="data">Data safety</option></select></label><label>Member<select id="timelineMemberFilter"><option value="all">All members</option></select></label><label>Range<select id="timelineRangeFilter"><option value="all">All loaded</option><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="90">Last 90 days</option><option value="365">Last year</option></select></label></div><div id="systemTimelineList" class="system-timeline-list"></div><div class="timeline-load-row"><button id="timelineLoadOlderButton" class="secondary-button" type="button">Load older</button></div></article>';
  ...
  setRoute=function(route){
    ...
    document.querySelector('#openFrontManager').hidden=true;
